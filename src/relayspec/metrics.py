...
def paired_binary_delta_summary(
    ...
    *,
    ...
    """Summarize a paired binary-quality difference with a task bootstrap."""
    if set(reference) != set(candidate):
        raise ValueError("reference and candidate task IDs must match exactly")
    if not reference:
        raise ValueError("paired binary summary requires at least one task")
    if samples <= 0:
        raise ValueError("bootstrap samples must be positive")
    task_ids = sorted(reference)
    differences = [int(candidate[key]) - int(reference[key]) for key in task_ids]
    ...
    return {
        ...
        "reference_accuracy": statistics.fmean(
            bool(reference[key]) for key in task_ids
        ),
        "candidate_accuracy": statistics.fmean(
            bool(candidate[key]) for key in task_ids
        ),
        ...
        "candidate_only_passes": sum(difference == 1 for difference in differences),
        "reference_only_passes": sum(difference == -1 for difference in differences),
...
def paired_bootstrap_summary(
    ...
    *,
    ...
    bootstrap_groups: dict[tuple[str, int], list[dict[str, dict[str, Any]]]] = {}
    has_mtbench = False
    for pair in pairs:
        reference_row = pair[reference_method]
        problem_id = str(reference_row["problem_id"])
        repetition = int(reference_row.get("repetition", 0))
        if str(reference_row.get("benchmark", "")) == "mtbench":
            has_mtbench = True
            marker = problem_id.rfind("/turn")
            conversation_id = problem_id[:marker] if marker >= 0 else problem_id
            cluster_key = (conversation_id, repetition)
        else:
            cluster_key = (problem_id, repetition)
        bootstrap_groups.setdefault(cluster_key, []).append(pair)
    bootstrap_clusters = list(bootstrap_groups.values())

    has_request_time = all(
        "request_seconds" in pair[method] for pair in pairs for method in method_names
    )
    has_hashes = all(
        "output_hash" in pair[method] for pair in pairs for method in method_names
    )
    has_accuracy = all(
    ...
    point_reference_tps, point_reference_request_tps, point_reference_accuracy = totals(
        pairs, reference_method
    )
    source_method = next(
        (
            name
    ...
    result: dict[str, Any] = {
        ...
        "bootstrap_clusters": len(bootstrap_clusters),
        "bootstrap_unit": (
            "paired request; MT-Bench conversation" if has_mtbench else "paired request"
        ),
        "reference_method": reference_method,
        "methods": {},
    }
    for method in method_names:
        ...
        if all("acceptance_lengths" in row for row in selected_rows):
            ...
            method_result["acceptance_survival_by_position"] = {
                str(position): sum(length >= position for length in cycle_lengths)
                / len(cycle_lengths)
        ...
        # Some inherited proposer APIs expose no synchronized first-token
        # event and use 0.0 as a compatibility placeholder.  Omitting TTFT is
        # more truthful than aggregating that sentinel into a measured value.
        if all(
            "time_to_first_token_seconds" in row
            and float(row["time_to_first_token_seconds"]) > 0.0
            for row in selected_rows
        ):
        ...
        if all("request_seconds" in row for row in selected_rows):
            request_latencies = [float(row["request_seconds"]) for row in selected_rows]
            method_result["request_latency_p50_seconds"] = _percentile(
                request_latencies, 0.50
            )
            method_result["request_latency_p95_seconds"] = _percentile(
                request_latencies, 0.95
            )
        ...
        if has_hashes:
            exact = sum(
                pair[method]["output_hash"] == pair[reference_method]["output_hash"]
                for pair in pairs
            )
            method_result["exact_sequence_match_rate_vs_reference"] = exact / len(pairs)
            if reference_method == "native_ar":
                method_result["exact_sequence_match_rate_vs_native_ar"] = method_result[
    ...
    for _ in range(samples):
        sampled_clusters = [
            bootstrap_clusters[generator.randrange(len(bootstrap_clusters))]
            for _ in bootstrap_clusters
        ]
        resample = [pair for cluster in sampled_clusters for pair in cluster]
    ...
    for method in method_names:
        ...
        if reference_method == "native_ar":
            summary["speedup_vs_native_ar_ci95"] = summary["speedup_vs_reference_ci95"]
    ...
    if has_hashes and source_method is not None:
        for method in method_names:
            exact = sum(
                pair[method]["output_hash"] == pair[source_method]["output_hash"]
                for pair in pairs
            )
            result["methods"][method]["exact_sequence_match_rate_vs_source_reuse"] = (
                exact / len(pairs)
            )
    return result
