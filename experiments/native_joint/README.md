...
The user requires at least10% end-to-end speedup over original DFlash and has prioritized rapid, radical decoding ideas after joint training reached only near parity. `radical_decode.py` and `run_radical.py` test inference policies with the unchanged native weights: verification length, history lookup, conditional multi-branch verification, and recycling unused proposals. These screens extend the separate native study; they are not RelaySpec results. Every proposed token is still target-verified, and numerical output differences are measured explicitly. A small-screen gain is only a promotion signal: repeated broader tests and the reserved128-request/2,048-token comparison are required. That frozen comparison is complete: 128 requests, two timing repeats, four methods, plus an AR quality reference. See `reports/DECISION.md` for the final decision and `reports/CONFIRMATION.md` for measurements.

Final evidence: original DFlash **169.5 TPS**; jointly trained compact linear drafter **159.2 TPS (−6.1%)**; compact + DDTree47 **185.9 TPS (+9.7%)**; full drafter + DDTree63 **199.3 TPS (+17.6%)**. Full DDTree's paired 95% ratio interval is **[1.155, 1.198]**, entirely above the 10% target. The compact-plus-tree interval is **[1.070, 1.123]** and does not establish a 10% gain. These are aggregate end-to-end measurements at a 2,048-token cap, including prefill, on 32 requests per workload. Joint compression adds no gain. The DDTree improvement is an existing-method result, not a new contribution. Output agreement and task scoring are reported separately; neither bitwise equivalence nor universal quality preservation is claimed.
...
Run `analyze_radical.py` and `analyze_progress.py` from this folder to regenerate all positive and negative screens. `reports/PROGRESS.md` records the current queue and decisions. The frozen 128-request confirmation is complete. Full DDTree exceeds the 10% target; none of our tested changes improves on it. All positive and negative evidence remains in this standalone study.
