...
The sandbox binds only system runtime directories, installed Python packages, the private grading dependencies and the vendored evaluation source as read-only. The home directory and network are absent. Limits are20 CPU seconds,30 wall seconds,768MiB address space and1MiB output-file size. Correct/incorrect code, an infinite loop, hidden home access and a known math answer are tested by invoking `quality.py` directly. Math symbolic parsing also runs inside this sandbox.
...
Scoring protocol v2 supersedes v1 after SIGKILL resource failures were observed on valid answers. All methods are rescored under the same revised limits; v1 source, protocol and raw scores are preserved in reports/quality-v1. Resource failures are unscored, never counted incorrect; the final quality table requires no unscored math/code outputs.
