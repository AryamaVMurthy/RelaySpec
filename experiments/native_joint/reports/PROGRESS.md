...
**29298** (v32) is active: compile only proposal-side linear layers, comparing BF16 versus group128 int4, each with draft-only or draft+separate-head compilation. All four lanes have540-second bounds. BF16 compiled controls are necessary before attributing any benefit to quantization. The target stays BF16 and uncompiled in every arm. Initial packing, reload and compilation are recorded separately; any later compilation inside measured generations remains included in reported time. The compiled probe must reproduce eager tokens/acceptance.
