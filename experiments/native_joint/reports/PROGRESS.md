...
**29301** (v33) is active: compile only proposal-side linear layers, comparing BF16 versus group128 int4, each with draft-only or draft+separate-head compilation. All four lanes have540-second bounds. BF16 compiled controls are necessary before attributing any benefit to quantization. The target stays BF16 and uncompiled in every arm. Initial packing, reload and compilation are recorded separately; any later compilation inside measured generations remains included in reported time. The compiled probe must reproduce eager tokens/acceptance.

Next architectural hypothesis, **not implemented or run**: inject a short predicted token prefix midway through the existing five draft layers, then let the remaining layers refine the tail within the same pass. This targets the second-pass refiner's overhead while adding only a small intermediate head projection. Test an alpha0 exact-native control, auxiliary intermediate prediction supervision, and alpha/prefix/depth variants on disjoint native-generated sequences. Any straight-through embedding treatment must have an explicit train/inference forward-equivalence check. All committed tokens still require the unchanged target verifier. Do not describe this as novel before comparing relevant prior work, or as promising evidence before measurements.
