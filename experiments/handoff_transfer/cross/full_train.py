...
def main(a):
    ...
    assert len(cache)==4096 and a.steps in (100,500,512,2000)
    ...
    for step in range(first_step,a.steps):
        ...
        torch.manual_seed(42000+step*100)
        ...
        opt.step()
        history.append({'step':step+1,'rank_local_mean_loss':sum(losses)/len(losses)})
        if (step+1)%100==0:
            if rank==0:
    ...
    if rank==0:
        ...
        write(out/'summary.json',{'scope':'full-data cross-family fit; decoding measurement separate','records':len(cache),
            'steps':a.steps,'optimizer_steps':a.steps,'presentations':8*a.steps,'processed_examples':8*a.steps,'anchors_limit':NUM_ANCHORS,'anchors_per_example':NUM_ANCHORS,'world_size':world,'batch_per_gpu':8,'gradient_accumulation':1,'seed':42,'variant':a.kind,'checkpoint':'final','objective':OBJECTIVE,
            'label_units':'source tokens','context_units':'target tokens','lr':a.lr,
            'training_seconds':training_seconds,'training_gpu_hours':training_seconds*world/3600,'actual_anchors_rank0':anchor_count,'records_rank0':record_count,'objective_chunk_blocks':raw.objective_chunk_blocks,'peak_cuda_bytes_rank0':torch.cuda.max_memory_allocated(),
...
if __name__=='__main__':
    ...
    p.add_argument('--index',type=Path,required=True);p.add_argument('--steps',type=int,choices=[100,500,512,2000],required=True);p.add_argument('--lr',type=float,default=1e-4);main(p.parse_args())
