...
def batches(tag,epoch,rank,world):
 ...
 for pi,path in enumerate(paths):
  ...
  for start in range(0,len(indices),2):
   ...
   ids=torch.zeros((len(ii),S),dtype=torch.long);h=torch.zeros((len(ii),S,12800),dtype=torch.bfloat16);mask=torch.zeros((len(ii),S))
   for j,(i,x) in enumerate(zip(ii,data)):
    n=len(x['full_ids']);ids[j,:n]=torch.tensor(x['full_ids']);h[j,:n]=s['features'][i];mask[j,len(x['prompt_token_ids']):n]=1
...
def main(a):
   ...
   if accum==2:
    ...
    if step % a.snapshot_every == 0:
     if rank==0:
      ...
      put(out/'summary.json',{'processed_examples':processed,'optimizer_steps':step,'unique_cached_examples':4096,'training_seconds':time.perf_counter()-t0,'training_gpu_hours':(time.perf_counter()-t0)*world/3600,'checkpoint':'fixed cumulative-example milestone','objective':'AUF','anchors_per_example':8,'seed':a.seed,'lr':a.lr,'total_schedule_steps':total_steps})
     dist.barrier()
    ...
    if rank==0 and (step%10==0 or step==1):
     status={'epoch':epoch+1,'step':step,'total_steps':total_steps,'loss_local':float(loss),'elapsed_seconds':time.perf_counter()-t0}
 ...
 if rank==0:
  ...
  put(out/'summary.json',{'trainable_parameters':n,'optimizer_steps':step,'training_seconds':seconds,'training_gpu_hours':seconds*world/3600,'world_size':world,'seed':a.seed,'variant':a.kind,'history':history,'peak_cuda_bytes_rank0':torch.cuda.max_memory_allocated(),'lr':a.lr,'bias_lr':.01,'initialization':'base draft, zero bias, PEFT random A zero B; seed42','processed_examples':step*8,'total_schedule_steps':total_steps,'objective':'accept-until-fail, first failure included, detached prefix weights','anchors_per_example':8,'batch_per_gpu':2,'gradient_accumulation':2,'checkpoint':'final','source_sha256':{p.name:sha(p) for p in R.glob('*.py')}})
