...
def main(a):
   ...
   if accum==2:
    ...
    if step % a.snapshot_every == 0:
     if rank==0:
      ...
      put(out/'summary.json',{'processed_examples':processed,'optimizer_steps':step,'unique_cached_examples':4096,'training_seconds':time.perf_counter()-t0,'training_gpu_hours':(time.perf_counter()-t0)*world/3600,'checkpoint':'fixed cumulative-example milestone','objective':'uniform CE','anchors_per_example':8,'seed':a.seed,'lr':a.lr,'total_schedule_steps':total_steps})
     dist.barrier()
 ...
 if rank==0:
  ...
  put(out/'summary.json',{'trainable_parameters':n,'optimizer_steps':step,'training_seconds':seconds,'training_gpu_hours':seconds*world/3600,'world_size':world,'seed':a.seed,'variant':a.kind,'history':history,'peak_cuda_bytes_rank0':torch.cuda.max_memory_allocated(),'lr':a.lr,'bias_lr':.01,'initialization':'ZIP epoch-3 initialization; see architecture model contract; seed42','processed_examples':step*8,'total_schedule_steps':total_steps,'objective':'uniform valid-position CE; matched handoff ablation','anchors_per_example':8,'batch_per_gpu':2,'gradient_accumulation':2,'checkpoint':'final','source_sha256':{p.name:sha(p) for p in R.glob('*.py')}})
