...
def main(a):
    ...
    if a.kind!='fusion_r56':folded=draft.fc.folded()
    ...
    assert error<1e-6
    assert (dest/'config.json').read_bytes()==(Path(TRANSFER['normal_export'] if a.kind=='normal_ce' else TRANSFER['base_export'])/'config.json').read_bytes()
