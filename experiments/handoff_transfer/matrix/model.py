...
def build(kind):
    ...
    if kind=='fusion_r56':
        ...
        draft.fc=projection_type(native_state['fc.weight'],[maps[f'maps.{i}.weight'] for i in range(5)])
        ...
        initial=f0
        if kind=='normal_ce':
            normal=Path(TRANSFER['normal_export'])
            assert sha(normal/'model.safetensors')==TRANSFER['normal_sha256']
            initial=load_file(str(normal/'model.safetensors'))['fc.weight']
            initialization={'normal_export':str(normal),'normal_sha256':TRANSFER['normal_sha256']}
        elif kind=='dense_fresh':
            initial=torch.empty_like(f0,dtype=torch.float32)
            nn.init.kaiming_uniform_(initial,a=math.sqrt(5))
            initialization={'method':'fresh Kaiming dense matrix'}
        input_eps=json.loads((Path(TRANSFER['normal_export'])/'config.json').read_text())['relayspec_normal_input_eps'] if kind=='normal_ce' else 1e-6
        draft.fc=DenseFusion(initial,normalize_input=kind=='normal_ce',eps=input_eps)
    ...
    expected=56*(INPUT_WIDTH+width) if kind=='fusion_r56' else (5*56*(INPUT_WIDTH//5+width) if kind=='five_ba56' else INPUT_WIDTH*width)
    assert n==expected
...
def export(model,cfg,kind,dst):
    ...
    state={k.replace('.base_layer',''):v.detach().to(dtype=torch.bfloat16,device='cpu').contiguous()
           for k,v in draft.state_dict().items() if '.lora_' not in k and not (kind!='fusion_r56' and k.startswith('fc.'))}
    if kind!='fusion_r56':state['fc.weight']=draft.fc.folded().detach().to(dtype=torch.bfloat16,device='cpu').contiguous()
    ...
    config_source=Path(TRANSFER['normal_export'] if kind=='normal_ce' else TRANSFER['base_export'])/'config.json'
    (dst/'config.json').write_text(config_source.read_text())
