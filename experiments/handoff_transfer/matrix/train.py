"""One GPU per independent variant; batch8, one update per batch."""
...
def batches(tag,epoch,rank,world):
 ...
 for pi,path in enumerate(paths):
  ...
  for start in range(0,len(indices),8):
   ...
   if (pi*4+start//8)%world!=rank:continue
   ii=indices[start:start+8];assert len(ii)==8;data=[s['rows'][i] for i in ii];S=max(len(x['full_ids']) for x in data)
...
def main(a):
 rank=int(os.environ['LOCAL_RANK']);world=int(os.environ['WORLD_SIZE']);assert world==1
 torch.cuda.set_device(rank);dist.init_process_group('nccl');torch.manual_seed(a.seed)
 raw,cfg=build(a.kind);raw.train();model=raw
 parameters=[p for p in raw.parameters() if p.requires_grad]
 anchor_count=0;records_count=0;counts_complete=True
 ...
 if resume_path.exists():
  ...
  counts_complete=resume.get('counts_complete','anchor_counts' in resume)
   ...
   if accum==1:
    ...
    if step%100==0:
     ...
     if rank==0:
      ...
      state={'parameters':{k:p.detach().cpu() for k,p in raw.draft_model.named_parameters() if p.requires_grad},'optimizer':opt.state_dict(),'step':step,'epoch':epoch,'next_batch':batch_index+1,'loss_sums':loss_totals,'anchor_counts':anchor_totals,'record_counts':record_totals,'counts_complete':counts_complete,'schedule_contract':{'lr':a.lr,'total_steps':total_steps,'seed':a.seed,'kind':a.kind,'objective':OBJECTIVE},'history':history,'elapsed_seconds':time.perf_counter()-t0,'anchor_rng':'manual seed derived from seed/epoch/rank/microbatch index before each forward'}
    ...
    if rank==0 and (step%10==0 or step==1):
     status={'epoch':epoch+1,'step':step,'total_steps':total_steps,'loss_local':float(loss),'anchor_count_rank0':anchor_count,'records_rank0':records_count,'objective_chunk_blocks':raw.objective_chunk_blocks,'peak_cuda_bytes_rank0':torch.cuda.max_memory_allocated(),'elapsed_seconds':time.perf_counter()-t0}
 ...
 if rank==0:
  ...
  put(out/'summary.json',{'trainable_parameters':n,'optimizer_steps':step,'training_seconds':seconds,'training_gpu_hours':seconds*world/3600,'world_size':world,'seed':a.seed,'variant':a.kind,'history':history,'objective_chunk_blocks':raw.objective_chunk_blocks,'peak_cuda_bytes_rank0':torch.cuda.max_memory_allocated(),'lr':a.lr,'bias_lr':.01,'initialization':json.loads((R/f'model-contract-{a.kind}.json').read_text())['initialization'],'processed_examples':step*8,'total_schedule_steps':total_steps,'objective':OBJECTIVE,'anchors_per_example':NUM_ANCHORS,'batch_per_gpu':8,'gradient_accumulation':1,'checkpoint':'final','actual_anchors_rank0':anchor_count if counts_complete else None,'records_rank0':records_count if counts_complete else None,'anchor_count_scope':'complete run' if counts_complete else 'legacy resume omitted historical counts; totals unavailable','anchor_limit_semantics':'distinct valid anchors; short records supply fewer','source_sha256':source_hashes})
