...
NEW_CAMPAIGN='optimized-all128-20260912'
...
def run(pilot=False):
    ...
    for family in ([cases[task][0]] if pilot else ['q8','cross']):
        ...
        if pilot:
            ...
            (data/'eval.json').write_text(json.dumps(read(base/'evaluation/eval.json')[:2]))
            (data/'warmup.json').write_text(json.dumps(read(base/'evaluation/warmup.json')[:1]))
            count=2;cap=128;workers=1;index=0
        ...
        for label in order:
            ...
            command=[sys.executable,'-u','-m','experiments.auf_vllm.family_benchmark',
                     ...
                     '--runtime-profile','optimized',*extra]
            ...
            compared=compare([paths['ar']],[path]);assert compared['count']==(2 if pilot else 32)
