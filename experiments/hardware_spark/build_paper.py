...
def main():
    ...
    counters={}
    for m in ["source","relay"]:
        read(f"counters-{m}-run/complete.json")
    ...
    fig,axes=plt.subplots(1,3,figsize=(11.3,3.15),gridspec_kw={"width_ratios":[1.6,.85,1.05]},layout="constrained")
    ...
    axes[2].set_xticks([0,1],["Source\nreuse","RelaySpec"]);axes[2].set_ylabel("Projected region time / token (ms)")
    axes[2].set_title("(c) Separate CUDA profile",loc="left",fontweight="bold")
    axes[2].legend(fontsize=7,frameon=False)
    for ax in axes:ax.grid(axis="y",alpha=.17);ax.set_axisbelow(True)
    for ext in ["pdf","png"]:fig.savefig(out/f"hardware_spark.{ext}",dpi=200,bbox_inches="tight")
    plt.close(fig)
    ...
    payload["output_sha256"]={p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [out/"hardware_spark.pdf",out/"hardware_spark.png",out/"hardware_spark_table.tex"]}
