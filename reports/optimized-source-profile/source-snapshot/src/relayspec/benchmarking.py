...
def model_load_plan(
    ...
    *,
    ...
    needs_source_provider = "naive_source_reuse" in selected
    needs_source_heads = bool(selected & {"relay_f", "relay_p"})
    if unload_source_trunk and needs_source_provider:
        raise ValueError(
            "cannot unload the source trunk while benchmarking naive_source_reuse"
        )
    return {
...
def optional_generation_stats(stats: Any) -> dict[str, Any]:
    ...
    for name in (
        ...
        if hasattr(stats, name):
            ...
            result[name] = list(value) if isinstance(value, (list, tuple)) else int(value)
    return result
