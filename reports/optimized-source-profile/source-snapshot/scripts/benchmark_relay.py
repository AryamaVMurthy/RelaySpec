...
def run_method(
    ...
    decode_seconds = float(stats.time_per_output_token) * int(stats.num_output_tokens)
    ...
    row = {
        ...
        "time_to_first_token_seconds": float(stats.time_to_first_token),
...
def main() -> None:
    ...
    unload_source_trunk = bool(
        getattr(config.benchmark, "unload_source_trunk", False)
    )
    ...
    draft = draft_class.from_pretrained(
        config.proposer.id,
    ...
    if load_plan["load_native_target_draft"]:
        ...
        native_target_draft = draft_class.from_pretrained(
    ...
    if rank == 0:
        ...
        for worker in range(world_size):
            with (output_dir / f"benchmark-rank{worker}.jsonl").open(
                ...
                merged.extend(json.loads(line) for line in worker_stream if line.strip())
        summary = aggregate_rows(merged, methods=method_names)
        if "naive_source_reuse" in summary["methods"] and "relay_p" in summary["methods"]:
            summary["methods"]["relay_p"]["speedup_vs_naive_source_reuse"] = (
                summary["methods"]["relay_p"]["decode_tokens_per_second"]
                / summary["methods"]["naive_source_reuse"]["decode_tokens_per_second"]
            )
