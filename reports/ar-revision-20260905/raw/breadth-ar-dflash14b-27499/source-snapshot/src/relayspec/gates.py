...
def select_profile_provider(
    *, predicted_speedup: float, speed_ci_lower: float | None = None
) -> str:
    """Select relay only above the algebraic and optional confidence bounds.

    Both boundaries are the source-reuse speedup of one.  They are not tuned
    quality thresholds.  Omitting ``speed_ci_lower`` gives the pure Amdahl
    direction policy used to audit the cross-task matrix; supplying it gives
    the conservative calibrated deployment policy.
    """
    if predicted_speedup <= 1.0:
        return "source"
    if speed_ci_lower is not None and speed_ci_lower <= 1.0:
        return "source"
    return "relay"
...
def evaluate_development_gate(
    ...
    return {
        "passed": not failures,
        "selected_provider": "relay" if not failures else "source",
        "failures": failures,
        "criteria": {
