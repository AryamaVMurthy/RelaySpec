...
def paired_binary_delta_summary(
    ...
    *,
    ...
    return {
        ...
        "reference_accuracy": statistics.fmean(bool(reference[key]) for key in task_ids),
        "candidate_accuracy": statistics.fmean(bool(candidate[key]) for key in task_ids),
...
def paired_bootstrap_summary(
    ...
    *,
    ...
    result: dict[str, Any] = {
        ...
        "bootstrap_unit": (
            "paired request; MT-Bench conversation"
            if has_mtbench
            else "paired request"
        ),
        "reference_method": reference_method,
    ...
    for method in method_names:
        ...
        if all("request_seconds" in row for row in selected_rows):
            request_latencies = [
                float(row["request_seconds"]) for row in selected_rows
            ]
