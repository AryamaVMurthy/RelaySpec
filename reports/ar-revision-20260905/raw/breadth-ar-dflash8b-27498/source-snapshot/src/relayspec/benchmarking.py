...
def resolve_revision(model_id: str, revision: str | None) -> str | None:
    """The revision to pass to `from_pretrained`, or None for a local path.

    `from_pretrained(path, revision=..., local_files_only=True)` still
    attempts an online hub lookup when `revision` is set, even if `path` is
    an existing local directory (e.g. a locally-produced fine-tuned
    checkpoint, as used for the policy-drift study): transformers only
    skips the network call when `revision` is `None`. A real HF repo ID is
    never a local path that happens to exist on this machine, so checking
    `Path(model_id).is_dir()` cannot misfire on one.
    """
...
def model_load_plan(
    ...
    *,
    ...
    needs_source_heads = bool(
        selected & {"relay_f", "relay_p", "relay_p_cross_family"}
    )
...
def eagle3_load_plan(methods: Sequence[str]) -> dict[str, bool]:
    """Minimize resident EAGLE components, especially for the 14B matrix."""
    selected = set(methods)
    source_reuse = "source_reuse_eagle3" in selected
    relayed = "relay_eagle3" in selected
    return {
        "load_source": source_reuse,
        "load_source_draft": source_reuse or relayed,
        "load_target_draft": "native_target_eagle3" in selected,
        "load_relay": relayed,
    }
