...
for precision in bfloat16 float16; do
  export RELAYSPEC_OUTPUT="$FAMILY_SCALE_CACHE/precision-cross-$precision"
  mkdir -p "$RELAYSPEC_OUTPUT"
  "$PY" -m torch.distributed.run --standalone --nproc_per_node=1 scripts/benchmark_mixed_precision_relay.py --config "reports/family-scale-20260907/precision-cross-$precision.yaml" > "$RELAYSPEC_OUTPUT/stdout.log" 2> "$RELAYSPEC_OUTPUT/stderr.log"
done
