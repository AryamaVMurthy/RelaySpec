...
from relayspec.native_compute import fit_linear_mlp, native_intervention
...
def main():
    ...
    bridges = {}
    if any(m["kind"] == "linear_mlp" for m in methods.values()):
        ...
        for i in layer_ids:
            ...
            for rank in sorted({m["rank"] for m in methods.values() if m["kind"] == "linear_mlp" and i in m["layers"]}):
                ...
                diagnostics.append({"layer": i, "rank": rank, "train_positions": len(x)-nval, "validation_positions": nval, "train_relative_mse": train_loss, "validation_relative_mse": val_loss})
                torch.save(module.state_dict(), args.output/f"bridge-layer{i}-rank{rank}.pt")
