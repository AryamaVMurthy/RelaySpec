...
- Nsight Systems GPU-projected ranges include GPU spans associated with each stage. They are not a union of kernel-busy intervals or an unprofiled wall-time decomposition. Nsight Compute replay describes at most eight filtered kernels per source/relay path, including attention kernels selected by the CUTLASS filter.
