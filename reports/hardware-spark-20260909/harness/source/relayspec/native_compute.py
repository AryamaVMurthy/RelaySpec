...
class PrunedMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        out = f(self.act_fn(f(x, self.gate)) * f(x, self.up), self.down, self.bias)
        return out if self.correction is None else out + self.correction(x)


def fit_pruned_mlp(original, x, y, fraction, rank, folded=False):
    ...
    if folded and width >= 256:
        width = width // 256 * 256
    indices = scores.topk(width).indices.sort().values
    module = PrunedMLP(original, indices)
    if rank:
        if folded:
            retained = activations[:, indices].float()
            correction, _ = fit_linear_mlp(retained, y-module(xb).float(), rank)
            # No additional inference projection: reconstruct the missing update
            # from retained nonlinear activations and fold into the output matrix.
            module.down.add_((correction.down.float() @ correction.up.float()).T.to(module.down.dtype))
            module.bias = correction.bias.clone()
        else:
...
def native_intervention(draft, spec, bridges=None):
    ...
    try:
        ...
        elif kind in {"zero_mlp", "linear_mlp", "pruned_mlp", "folded_mlp"}:
            for i in selected:
                if kind == "zero_mlp":
                    ...
                    draft.layers[i].mlp = bridges[(i, "folded" if kind == "folded_mlp" else "pruned", spec["fraction"], spec["rank"])]
        ...
        elif kind == "prefix_refine":
            head, embedding = bridges["native_head"], bridges["native_embedding"]
            prefix = int(spec["prefix"])
            if prefix < 1:
                raise ValueError("Refinement needs at least one guessed prefix token")

            def refined(*args, **kwargs):
                cache = kwargs["past_key_values"]
                start = cache.get_seq_length()
                first = original_forward(*args, **kwargs)
                count = min(prefix, first.shape[1]-1)
                guesses = head(first[:, 1:1+count]).argmax(-1)
                noise = kwargs["noise_embedding"].clone()
                noise[:, 1:1+count] = embedding(guesses)
                # Roll back the draft only: the second pass inserts exactly the
                # same context positions once, followed by revised noise tokens.
                cache.crop(start)
                second = original_forward(*args, **{**kwargs, "noise_embedding": noise})
                # Guessed anchors retain their first-pass predictions. Only the
                # still-masked suffix takes predictions from the refinement pass.
                return torch.cat((first[:, :1+count], second[:, 1+count:]), dim=1)

            draft.forward = refined
        elif kind not in {"native", "block_size"}:
