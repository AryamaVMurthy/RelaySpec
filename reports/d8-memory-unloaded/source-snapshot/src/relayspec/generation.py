...
def matched_full_target_dflash_generate(
    ...
    *,
    ...
    cache = DualCacheState.create(source_provider.source.config, native_target.config)
    ...
    while start < max_length - 1:
        ...
        cache.begin_cycle(block_size)
        with _profile_region(profile_recorder, "verification_source_trunk"):
            source_verification = source_provider(
                block_ids,
                cache_state=cache,
            )
        ...
        proposer_context = torch.cat(source_verification.source_taps, dim=-1)[
            :, :accepted, :
        ]
