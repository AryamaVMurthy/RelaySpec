...
def main():
    ...
    def native(ids, cap, intervention=None):
        block_size = (intervention or {}).get("block_size", spec.get("block_size", 16))
        return generate(draft, target, input_ids=ids, max_new_tokens=cap, stop_token_ids=stop_ids, temperature=0.0, block_size=block_size, return_stats=True)

    bridges = {"native_head": target.lm_head, "native_embedding": target.model.embed_tokens}
    ...
    if any(m["kind"] in {"linear_mlp", "pruned_mlp", "folded_mlp"} or (m["kind"] == "shared_context" and m.get("rank", 0)) for m in methods.values()):
        ...
        for i in layer_ids:
            ...
            pruning = sorted({(m["kind"], m["fraction"], m["rank"]) for m in methods.values() if m["kind"] in {"pruned_mlp", "folded_mlp"} and i in m["layers"]})
            for kind, fraction, rank in pruning:
                with torch.no_grad():
                    module, indices = fit_pruned_mlp(draft.layers[i].mlp, x[:-nval], y[:-nval], fraction, rank, folded=kind == "folded_mlp")
                    prediction = module(x[-nval:].to(torch.bfloat16)).float()
                    val_loss = ((prediction-y[-nval:]).square().sum(-1)/y[-nval:].square().sum(-1).clamp_min(1e-8)).mean().item()
                bridges[i, "folded" if kind == "folded_mlp" else "pruned", fraction, rank] = module
                diagnostics.append({"layer": i, "kind": kind, "fraction": fraction, "rank": rank, "retained_neurons": indices.cpu().tolist(), "train_positions": len(x)-nval, "validation_positions": nval, "validation_relative_mse": val_loss})
                path = storage/f"{kind}-layer{i}-fraction{fraction}-rank{rank}.pt"
                # Original nonlinear weights are already pinned: only save the
                # selected indices and learned correction, not model copies.
                torch.save({"indices": indices.cpu(), "correction": None if module.correction is None else module.correction.state_dict(), "folded_down": module.down.cpu() if kind == "folded_mlp" else None, "bias": module.bias}, path)
                checkpoint_index[path.name] = {"path": str(path), "sha256": digest(path)}
        if context_ranks:
    ...
    for name, intervention in methods.items():
        with native_intervention(draft, intervention, bridges):
            native(encode(records[0]), 16, intervention)
    rows = []
        ...
        for idx, record in enumerate(records):
            ...
            for name in rotate_methods(tuple(methods), idx):
                with native_intervention(draft, methods[name], bridges):
                    ...
                    stats = native(ids, spec["max_new_tokens"], methods[name])
                    torch.cuda.synchronize()
                    elapsed = time.perf_counter()-t0
                output = stats.output_ids[0, ids.shape[1]:].cpu()
                row = {"method": name, "benchmark": record["benchmark"], "problem_id": record["problem_id"], "repetition": 0, "turn_index": 0, "input_tokens": ids.shape[1], "output_tokens": len(output), "request_seconds": elapsed, "output_token_ids": output.tolist(), "output_hash": hashlib.sha256(output.to(torch.int32).numpy().tobytes()).hexdigest(), "acceptance_lengths": stats.acceptance_lengths, "acceptance_length": sum(stats.acceptance_lengths)/len(stats.acceptance_lengths), "target_calls": len(stats.acceptance_lengths), "draft_calls": len(stats.acceptance_lengths) * (2 if methods[name]["kind"] == "prefix_refine" else 1), "completion": tokenizer.decode(output, skip_special_tokens=True)}
                rows.append(row)
                stream.write(json.dumps(row)+"\n"); stream.flush()
            print(json.dumps({"event": "request_done", "index": idx, "seconds": time.perf_counter()-started}), flush=True)
    # Separate diagnostic pass: event hooks never affect reported throughput.
    if spec.get("profile_methods"):
        profile = {}
        for name in spec["profile_methods"]:
            events, handles = [], []
            for label, module in [("target", target), ("draft", draft)]:
                stack = []
                def begin(module, args, kwargs, label=label, stack=stack):
                    start = torch.cuda.Event(enable_timing=True)
                    start.record()
                    stack.append(start)
                def end(module, args, kwargs, output, label=label, stack=stack):
                    stop = torch.cuda.Event(enable_timing=True)
                    stop.record()
                    events.append((label, stack.pop(), stop))
                handles.append(module.register_forward_pre_hook(begin, with_kwargs=True))
                handles.append(module.register_forward_hook(end, with_kwargs=True))
            try:
                with native_intervention(draft, methods[name], bridges):
                    torch.cuda.synchronize()
                    t0 = time.perf_counter()
                    stats = native(encode(records[0]), min(spec["max_new_tokens"], 512), methods[name])
                    torch.cuda.synchronize()
                    wall_ms = (time.perf_counter()-t0)*1000
                timings = {label: [start.elapsed_time(stop) for kind, start, stop in events if kind == label] for label in ["target", "draft"]}
                profile[name] = {"wall_ms": wall_ms, "module_stream_ms": timings, "scope": "Separate single-request CUDA-stream event diagnostic, includes stream gaps; not pure kernel time or benchmark timing. First target call includes prefill."}
            finally:
                for handle in handles:
                    handle.remove()
