...
def main():
    ...
    assert digest(base / "manifest32.json") == protocol["manifest_sha256"]
    frozen_ids = {
        r["problem_id"]
    ...
    for declaration in protocol["waves"]:
        ...
        for i, spec in enumerate(json.loads(wave.read_text())["lanes"]):
            ...
            assert (
                seed
                == declaration["sampling_seed_base"]
                == actual["benchmark"]["sampling_seed_base"]
            )
            manifest = repo / config["benchmark"]["manifest_path"]
            assert manifest == base / f"shard{declaration['shard']}.json"
            ...
            for r in rows:
                ...
                if r["method"] == "relay_base":
                    assert (
                        ...
                        == protocol["checkpoints"][spec["checkpoint"]]
                    )
                r.update(
            ...
            for p in [
                ...
                inputs[str(p.relative_to(repo))] = digest(p)
    assert len(cells) == 4 and sum(map(len, cells.values())) == 768
        ...
        results.append(
            dict(
                ...
                ar_reference=clustered_summary(
                    rows, "native_ar", protocol["sampling_seed_bases"]
                ),
                native_reference=clustered_summary(
                    rows, native, protocol["sampling_seed_bases"]
                ),
            )
        )
    output = dict(
    ...
    (root / "sampling-breadth-summary.json").write_text(
        json.dumps(output, indent=2) + "\n"
    )
    (root / "sampling-breadth-scored.jsonl").write_text(
