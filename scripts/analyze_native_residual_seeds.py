...
output = root / 'native-residual-seeds-summary.json'
analyze = runpy.run_path('scripts/analyze_compact_interface_data.py')['analyze']
result = analyze(28550, 55, output)
for i, cell in enumerate(result['results']):
    trial = cell['fit']['trial']
    p = root / 'run-28550' / f'lane{i}' / 'fitting' / trial['name'] / 'initialization.json'
