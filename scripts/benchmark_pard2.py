"""Pinned PARD-2 TD/TI comparison with identical verifier-observed replicas."""
...
def main():
    ...
    setup = json.loads(Path(os.environ["PARD_SETUP_GATE"]).read_text())
    if setup["status"] != "pass" or setup["models"] != config["models"]:
        raise ValueError("PARD-2 model setup gate differs from declaration")
    source_config = config["models"]
    ...
    source = Path(os.environ["PARD_SOURCE"]) / "pard/pard2_infer.py"
    observed_class = load_instrumented_pard(source, trace_decisions=True, version=2)
    plain_class = load_instrumented_pard(source, version=2)
    paths = {
    ...
    infer.log = False
    if not infer.v2:
        raise ValueError("checkpoint does not enable PARD-2 target conditioning")
    infer.get_model(
    ...
    wrapped_draft = infer.model_draft
    independent_draft = wrapped_draft.base_model
    independent_draft.forward_gen = independent_draft.forward
    ...
    with (output / f"benchmark-rank{rank}.jsonl").open("w") as stream:
        for index, record in enumerate(records):
            ...
            for name in order:
                infer.v2 = name != "pard2_ti"
                infer.model_draft = wrapped_draft if infer.v2 else independent_draft
                if name == "native_ar":
                    eager_ar(
                        infer.model,
                        infer.target_cache,
