...
run = root / "run-28533"
wave_path = Path("configs/autoresearch/20260907/wave43.json")
wave = json.loads(wave_path.read_text())
job = next(
    j for j in json.loads((root / "jobs.json").read_text())["jobs"] if j["id"] == 28533
)
...
for i, (spec, task) in enumerate(
    ...
    assert transform["parent_sha256"] == spec["checkpoint_sha256"][spec["checkpoint"]]
...
(root / "native-initialization-transfer-summary.json").write_text(
