...
def main():
    root = Path("reports/autoresearch-20260907/run-28605")
    protocol_path = Path(
        "configs/autoresearch/20260907/code-calibration-v1/breadth-protocol.json"
    )
    wave_path = Path("configs/autoresearch/20260907/wave95.json")
    ...
    job = next(j for j in ledger["jobs"] if j["id"] == 28605)
    ...
    domain_comparisons = {
        m: summarize(all_rows, reference=m)
        for m in ["relay_math_2layers", "relay_math_3layers", "relay_code_2layers"]
    }
    summary = dict(
    ...
    Path(
        "reports/autoresearch-20260907/code-calibration-breadth-summary.json"
