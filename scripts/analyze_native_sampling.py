...
def main():
    ...
    parser.add_argument("--job", type=int, default=28575)
    parser.add_argument("--wave", type=int, default=80)
    ...
    for i, spec in enumerate(specs):
        ...
        methods.update(spec.get("candidates", {}))
        if spec.get("include_ar"):
            methods.add("native_ar")
        ...
        assert len(rows) == 8 * len(methods) and {
        ...
        for method, checkpoint in {
            ...
            **spec.get("candidates", {}),
            ...
            variant = campaign["variants"][method]
            assert (
                variant.get("checkpoint", variant.get("checkpoint_path")) == checkpoint
            )
            assert (
                variant.get("sha256", variant.get("checkpoint_sha256"))
                == spec["checkpoint_sha256"][checkpoint]
            )
        assert expected["benchmark"]["sampling_seed_base"] == 1732
        for p in [
            path,
        ...
        cells.append(
            dict(
                study_role="retargeting"
                if spec.get("include_ar")
                else "native_compression",
                family=spec["family"],
    ...
    for cell in cells:
        ...
        if any(r["method"] == "native_ar" for r in rows):
            cell["ar_reference"] = summarize(rows, reference="native_ar")
        lookup = {(r["problem_id"], r["method"]): r for r in rows}
        ids = sorted(common_ids)
        cell["paired_accuracy"] = {
            m: paired_accuracy_interval(
                [lookup[p, m]["correct"] for p in ids],
                [lookup[p, native]["correct"] for p in ids],
            )
            for m in cell["native_reference"]["methods"]
            if m != native
        }
        fields = ("output_hash", "acceptance_lengths", "target_calls", "draft_calls")
        cell["repacked_control_matches"] = {
            f: sum(
                lookup[p, native].get(f) == lookup[p, "relay_base"].get(f) for p in ids
            )
            for f in fields
        }
        cell["cap_counts"] = {
    ...
    result = dict(
        ...
        scope="Native projection compression: four descriptive cells reuse the same eight exposed questions. One explicit per-request sampling seed per cell.2048token cap. Different sampled outputs and lengths are expected. Paired request intervals condition on this seed, do not establish quality noninferiority or distributional exactness, and do not represent32 independent questions.",
    )
    (root / f"native-sampling-wave{args.wave}-summary.json").write_text(
        json.dumps(result, indent=2) + "\n"
    )
    (run / "native-sampling-scored.jsonl").write_text(
        "".join(json.dumps(r) + "\n" for r in scored)
    )
    for cell in cells:
        print(
            ...
            cell["temperature"],
            {
                m: dict(
                    ...
                    native_ratio=round(v["throughput_ratio"], 3),
                    correct=round(8 * v["accuracy"]),
                )
                for m, v in cell["native_reference"]["methods"].items()
            },
            cell["cap_counts"],
        )
